#Se uso un entorno virutal para instalar networkx y matplotlib
    ...
    with open(archivo, "r", encoding="utf-8") as f:
        for lineno, linea in enumerate(f, start=1):
            ...
            if not linea or linea.startswith("#"):
                continue
            if "->" not in linea:
                raise ValueError(f"Línea {lineno} inválida en la gramática")
...
def construir_arbol_sintactico(tokens):
    # Construye un árbol sintáctico para la expresión dada.
    # Asume que la gramática es:
    def parse_E(i):
        nodo_T, i = parse_T(i)
...
def hierarchy_pos(G, root, width=5., vert_gap=1, vert_loc=0, xcenter=0.9):
    pos = {}
